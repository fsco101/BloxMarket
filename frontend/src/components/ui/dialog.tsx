  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-3rem)] translate-x-[-50%] translate-y-[-50%] gap-6 rounded-xl border shadow-xl duration-200 sm:max-w-2xl md:max-w-3xl lg:max-w-4xl p-0 max-h-[calc(100vh-4rem)] overflow-hidden",
          className,
        )}
        {...props}
      >
...
function DialogHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-2.5 text-left px-8 pt-8 pb-4 border-b shrink-0",
        className
      )}
      {...props}
    />
...
function DialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col-reverse gap-3 sm:flex-row sm:justify-end px-8 py-6 border-t shrink-0 bg-muted/30",
        className,
      )}
      {...props}
    />
